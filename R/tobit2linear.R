...
tobit2linear <- function(x.train,
                    ...
                    tau_hyperprior = FALSE, alpha_tau = 1, beta_tau = 10, jointbetagamma = TRUE){
  ...
  # print("line 798 = ")
  alpha1hat <- solve(crossprod(wmat_train[uncens_inds,])) %*% crossprod(wmat_train[uncens_inds,],
  ...
  if(any(is.na(mutemp_z))){
    stop("804 any(is.na(mutemp_z))")
  }

  if(jointbetagamma){
    # print("line 815 = ")
    y_resids <- ystar[uncens_inds] - gamma1*(z[uncens_inds] - offsetz - mutemp_z[uncens_inds])
    Xresmat <- cbind(xmat_train[uncens_inds,] ,z[uncens_inds] - offsetz - mutemp_z[uncens_inds] )
  ...
  if(any(is.na(y_resids))){
    ...
    print("z[uncens_inds] = ")
    print(z[uncens_inds])
    print("mutemp_z[uncens_inds] = ")
    print(mutemp_z[uncens_inds])
    stop("862 any(is.na(y_resids))")

  }
  if(any(is.na(mutemp_y))){
  ...
  for(iter in 1:(n.iter+n.burnin)){
    ...
    if(any(is.na(mutemp_y))){
      ...
      stop("1042 any(is.na(mutemp_y))")
    }
    if(any(is.na(offsetz))){
      stop("1042 any(is.na(offsetz))")
    }
    if(any(is.na(z))){
      stop("1042 any(is.na(z))")
    }
    if(any(is.na(z_epsilon))){
      stop("1042 any(is.na(z_epsilon))")
    }
    if(any(is.na(y_epsilon))){
      stop("1045 any(is.na(y_epsilon))")
    }
    # print("temp_sd_z = ")
    # print(temp_sd_z)
    #
    ...
    # G0res <- rbind(cbind(Bvar_p, rep(0, nrow(Bvar_p))),
    #                t(rep(0,ncol(Bvar_p))), tau*phi1)
    #
    # Gbar <- solve(solve(G0res) + (1/phi1)*crossprod(Xresmat)  )
    # ghat <- solve(crossprod(Xresmat))%*% crossprod(Xresmat,  ystar[uncens_inds])
    # g_bar <- Gbar %*% (solve(G0res)%*%gamma0 + (1/phi1)*crossprod(Xresmat)%*% ghat)
    #
    # # sample beta vector and gamma simulataneously
    # betagamma_vec <- mvrnorm(1, mu = g_bar, Sigma = Gbar)
    #
    ...
    #
    #
    # alpha0hat <- solve(crossprod(wmat_train[cens_inds,])) %*% crossprod(wmat_train[cens_inds,], z[uncens_inds])
    # alpha1hat <- solve(crossprod(wmat_train[uncens_inds,])) %*% crossprod(wmat_train[uncens_inds,],
    #                                                                      z[uncens_inds] -
    #                                                                        (ystar[uncens_inds]  - mutemp_y)*gamma1/(phi1 + gamma1^2))
    #
    ...
    if(any(is.na(z_epsilon))){
      stop("any(is.na(z_epsilon))")
    }
    if(any(is.na(y_epsilon))){
      stop("any(is.na(y_epsilon))")
    }
    ...
    if(cov_prior == "Ding"){
      ...
      if(simultaneous_covmat == TRUE){
        ...
        if(!jointbetagamma){
          ...
          if(is.na(gamma_one)){
            print("((gamma0/tau) + crossprod(z_epsilon[uncens_inds] , y_epsilon[uncens_inds]   )[1] ) = ")
            print(((gamma0/tau) + crossprod(z_epsilon[uncens_inds] , y_epsilon[uncens_inds]   )[1] ))

            print("((1/tau) + crossprod(z_epsilon[uncens_inds])[1]) = ")
            print(((1/tau) + crossprod(z_epsilon[uncens_inds])[1]))
